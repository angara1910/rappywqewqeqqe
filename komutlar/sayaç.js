...
exports.run = async (client, message, args) => {
	if(!args[0]) {
		const embed = new Discord.RichEmbed()
			.setDescription(` **Lütfen Sayaçı Ayarlamam İçin Bir Sayı Belirt! (örnek: r!sayaç 100)**`)
	...
	if(isNaN(args[0])) {
		const embed = new Discord.RichEmbed()
			.setDescription(`Lütfen Sayaçı Ayarlamam İçin Bir Sayı Bilirt (örnek: r!sayaç 100`)
			.setColor(ayarlar.renk)
		message.channel.send({embed})
		return
	}
